...
mount_nfs_and_rsync() {
    ...
    gcloud compute ssh $NODE_NAME -- toolbox bash -c '"mount -t nfs -o nolock '$NFS_IP':'$NFS_TARGET_DIR' '$TARGET_DIR' && msrsync -p 4 -s 500M -f 1000 --progress --rsync -az '$SOURCE_DIR'/ '$TARGET_DIR'/"'
}
